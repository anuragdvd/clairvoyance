...
interface UseWebSocketsProps {
  ...
  playAudioData: (data: Uint8Array) => Promise<void>;
  resetAudio: () => void;
  onVisualizationData?: (visualizationText: string) => boolean;
}

export const useWebSockets = ({ sessionId, playAudioData, resetAudio, onVisualizationData }: UseWebSocketsProps) => {
    ...
    transcriptWSRef.current.onmessage = (event) => {
      try {
        ...
        if (data.type === 'transcript' && data.speaker && data.message) {
          ...
          // Check for visualization data in assistant messages
          if (data.speaker === 'assistant' && onVisualizationData) {
            const hasVisualization = onVisualizationData(data.message);
            if (hasVisualization) {
              console.log('🎨 Processed visualization data from transcript');
            }
          }
        }
      } catch (error) {
